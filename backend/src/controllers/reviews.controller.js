...
const submitReview = asyncHandler(async (req, res) => {
  ...
  if (limits && limits.reviews_completed >= DAILY_REVIEW_LIMIT) {
    const remaining = Math.max(0, DAILY_REVIEW_LIMIT - limits.reviews_completed);
    throw new AppError(
      `You've reached your daily limit of ${DAILY_REVIEW_LIMIT} code reviews. ` +
      `Your limit resets at midnight. Please try again tomorrow!`,
      429
    );
  }
...
const downloadReviewPDF = asyncHandler(async (req, res) => {
  ...
  doc
    .fontSize(11)
    .fillColor(lightGray)
    .text('AI-Powered Code Analysis by Learn.AI', 50, 100, { align: 'center', width: pageWidth });
  ...
  doc
    ...
    .fillColor(lightGray)
    .text(
      'Generated by Learn.AI Code Review | learnai.com',
      50,
      doc.y + 20,
